...
| global.icm.externalConnection | object | `{"enabled":false,"host":"","port":null}` | Defines the connection for an external ICM instance. If the connection is defined, ICM pod will not be deployed. |
...
| global.scaler.forceIcmInitImport | bool | `false` | Defines whether or not to import Scaler's user data into ICM. Set to 'true' if you intend to deploy this application after deploying ICM. |
| global.scaler.icmInitImport.accessRights | object | `{"admin":{"rightsStatus":"allow"},"allowImpersonate":{"rightsStatus":"allow"},"serverExportImport":{"rightsStatus":"allow"}}` | Defines which user access rights to grant to imported Scaler user during the initial import process. |
...
| global.sen | object | `{"enabled":false,"forceIcmInitImport":false,"passOverride":"","passOverrideSource":{"secretKey":"","secretName":"","useSecret":false},"userOverride":""}` | Available since the 15.0 version (except for the 15.2 version) of Scaler. |
...
| global.sen.forceIcmInitImport | bool | `false` | Defines whether or not to import SEN's user data into ICM. Set to 'true' if you intend to deploy this application after deploying ICM. |
...
| global.automation.userOverride | string | `nil` | Defines (in plain text) the username of an Automation user. If left undefined, the default user called automation is created. |
| global.automation.passOverride | string | `nil` | Defines (in plain text) the password of an Automation user. If left undefined, Automation's deployment will provide you with a random alphanumeric password. Use the 'passOverrideSource' variable instead if you wish to define the password using a Secret. |
...
| global.automation.portOverride | string | `nil` | Defines the port to run Automation on. If left undefined, the default port 10140 is used. |
| global.automation.forceIcmInitImport | bool | `false` | Defines whether or not to import Automation's user data into ICM. Set to 'true' if you intend to deploy this application after deploying ICM. |
| global.ingress.type | string | `""` | Defines the type of ingress instance you need to run. Use 'appgw' for Azure Kubernetes Service and 'hproxy' or 'nginx' for any other platform. [haproxy/nginx/appgw] |
