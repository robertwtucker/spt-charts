...
{{- $dbPassDefinition := dict "value" .Values.db.pass "source" .Values.db.passSource "secretName" (printf "%s-scaler-database" $applicationName) "secretKey" "dbPass" "envName" "DB_PASS" "envOnly" true -}}
{{- $dbUserDefinition := dict "value" .Values.db.user "source" .Values.db.userSource "secretName" (printf "%s-scaler-database" $applicationName) "secretKey" "dbUser" "envName" "DB_USER" "envOnly" true -}}
{{- $dbConnectStringDefinition := dict "value" .Values.db.connectionString "source" .Values.db.connectionStringSource "secretName" (printf "%s-scaler-database" $applicationName) "secretKey" "dbConnectionString" "envName" "DB_CUSTOM_CONNECTION_STRING" "envOnly" true -}}
