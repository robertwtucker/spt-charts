...
| global.zookeeper.enabled | bool | `false` | Defines whether to deploy a Zookeeper instance. _\[Not Implemented Yet\]_ |
| global.zookeeper.portOverride | string | `nil` | Defines the port to run the Zookeeper listener on. If left undefined, the default port 1445 is used. _\[Not Implemented Yet\]_ |
| ondemand.arsload.instances | list | `\[\]` | List of arsload deployment instances (one replica per input volume). |
...
| ondemand.fts.service.nodePorts.http | string | `""` | NodePort value to assign (.service.type must be `NodePort`). |
| ondemand.fts.service.type | string | `"ClusterIP"` | Defines the value for the Kubernetes service object \[ClusterIP/LoadBalancer/NodePort\]. |
...
| ondemand.restapi.service.nodePorts.http | string | `""` | NodePort value to assign (.service.type must be `NodePort`). |
| ondemand.restapi.service.type | string | `"ClusterIP"` | Defines the value for the Kubernetes service object \[ClusterIP/LoadBalancer/NodePort\]. |
